...
if (isset($_POST['status'])) {
    ...
    $bb=$conn->query("INSERT INTO `worker_attendance` ( `wrokerID`, `status`, `date`, `monthID`, `over_time`, `customer_id`) VALUES ('$wrokerID', '$status', '$date','$month', '$time', '$customerId')"); ?>
<script>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          ...
          $where = '';
          if ($_SESSION['type'] == 'worker') {
            $where = 'where worker_attendance.wrokerID = ' . $_SESSION['id'];
          } elseif ($_SESSION['type'] == 'customer') {
            $where = 'where worker_attendance.wrokerID in (select workerID from worker_assign where customer_id = ' . $_SESSION['id'] . ')';
          }
          $m = $conn->query("SELECT worker_attendance.*,worker.name,worker.iqama_number,month.month_name FROM `worker_attendance` JOIN worker ON worker_attendance.wrokerID=worker.id JOIN month ON worker_attendance.monthID=month.id $where ORDER BY worker_attendance.id DESC");
          //echo "SELECT worker_attendance.*,worker.name,worker.iqama_number,month.month_name FROM `worker_attendance` JOIN worker ON worker_attendance.wrokerID=worker.id JOIN month ON worker_attendance.monthID=month.id $where ORDER BY worker_attendance.id DESC";exit;
                ?>
          <div class="container">
