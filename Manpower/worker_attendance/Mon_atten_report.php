        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
                    ...
                    <?php
                    $workerId = $_GET['emp_id'];
                    $att = $conn->query("SELECT worker_attendance.*, worker.name as name FROM `worker_attendance` left join worker on worker.id = worker_attendance.wrokerID WHERE wrokerID='$workerId'");
  ...
  <h3>Monthly attendance report of " <?php echo $d_employee['name'] ?>"</h3> 
            <table class="table table-bordered">
              ...
              for ($x = 1; $x <= $dcount; $x++) { 
                  ...
                  $weekend=false;
                  $h = $conn->query("SELECT * FROM `worker_attendance` WHERE `date`<='$to_date' AND `date`>='$to_date'");
                  $hl = $h->fetch_assoc();
                  // print_r($hl);
                  // $holiday=$hl->title;
