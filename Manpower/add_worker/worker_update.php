...
$id=$_GET['id'];
 ...
 if(isset($_POST['submit'])){
    ...
    if($_FILES['photo']['name'] && $_FILES['iqama_photo']['name'] ){
        $worker = $conn->query("UPDATE `worker` SET `email`='$email', `password`='$password', `name`='$name',`iqama_number`='$iqama_numbe',`local_number`=' $local_number',`whatsapp_number`='$whatsapp_number',`photo`='$file_name',`iqama_photo`='$file_name',`current_address`='$current_address',`working_place`='$working_place ',`passport_number`='$passport_number',`status`='$status' WHERE id='$id'");
    } else {
        $worker = $conn->query("UPDATE `worker` SET `email`='$email', `password`='$password', `name`='$name',`iqama_number`='$iqama_numbe',`local_number`=' $local_number',`whatsapp_number`='$whatsapp_number',`current_address`='$current_address',`working_place`='$working_place ',`passport_number`='$passport_number',`status`='$status' WHERE id='$id'");
    };
                ...
                <div class="row">
                  ...
                  <div class="col-6">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="email" id="email" placeholder="Enter Email" value="<?php echo $result['name']; ?>">
                    </div>
                  </div>
                  <div class="col-6">
                    <div class="form-group">
                      <label for="Password">Password</label>
                      <input type="password" class="form-control" name="password" id="password" placeholder="Password" value="<?php echo $result['iqama_number']; ?>">
                    </div>
                  </div>
                  <div class="col-6">
                    <div class="form-group">
